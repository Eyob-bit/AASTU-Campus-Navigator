...
export function projectPointOnSegment(
  ...
  if (Math.abs(dLat) < 1e-9 && Math.abs(dLngRaw) < 1e-9) {
    const dist = calculateDistanceInMeters(pLat, pLng, aLat, aLng);
  ...
  const crossTrackMeters = calculateDistanceInMeters(pLat, pLng, projLat, projLng);
...
export class RouteProgressTracker {
  ...
  private computeCumulativeDistances() {
    ...
    for (let i = 0; i < this.polyline.length - 1; i++) {
      ...
      const dist = calculateDistanceInMeters(p1[0], p1[1], p2[0], p2[1]);
